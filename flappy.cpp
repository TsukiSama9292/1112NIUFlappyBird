...
void FlappyBird::setSFML(){ //用於設定初始值 
	...
	//視窗設定 
	window = new RenderWindow(VideoMode(1000, 600),"111_2NIUFlappy Bird"); //視窗設定 
...
void FlappyBird::pipeMove(){ //用於移動水管，與鳥的跳躍 
	...
	for (size_t i {}; i < pipes.size(); i++) { //利用size_t 遍歷物件  
		...
		if(pipes[i].getPosition().x == 440 && !addscore ){ //當管道比鳥更靠左側，且還未加分 
...
void FlappyBird::birdAnime(){ //設定鳥的動畫 
	...
	bird->setTextureRect(IntRect( 34 * (int)frame, 0, 34, 24 )); //設定圖示取樣範圍，前兩個值為圖片的座標，後兩值為取樣大小(寬、高) 
}
